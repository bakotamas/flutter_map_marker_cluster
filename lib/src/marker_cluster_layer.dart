  ...
  void _addLayers() {
    for (final marker in widget.options.markers) {
      _clusterManager.addLayer(
        ...
        _maxZoom,
        _minZoom,
        filter: widget.options.filterMarkers == true
      );
    }
    ...
    if (_zoomingOut && clusterNode.children.length > 1) {
      ...
      layers.add(
        MapWidget.static(
          ...
          translate: StaticTranslate(_mapCalculator, widget.options.filterMarkers == true ? clusterNode.markers.first : clusterNode),
          child: ClusterWidget(
            cluster: clusterNode,
            builder: widget.options.builder,
  ...
  VoidCallback _onClusterTap(MarkerClusterNode cluster) {
    return () async {
      ...
      var dest = widget.map.getBoundsCenterZoom(
        widget.options.filterMarkers == true ? LatLngBounds.fromPoints([cluster.markers.first.point]) : cluster.bounds,
