...
class _Derived {
  ...
  _Derived(List<MarkerOrClusterNode> children,
      ...
      {required bool recursively, bool filter = false}) {
    ...
    bounds = markerNodes.isEmpty
        ? null
        : filter
            ? LatLngBounds.fromPoints([markerNodes.first.point])
...
class MarkerClusterNode extends MarkerOrClusterNode {
  ...
  MarkerClusterNode(
      ...
      : super(parent: null) {
    _derived =
        _Derived(children, computeSize, recursively: false, filter: filter);
  }
  ...
  void recalculate({required bool recursively}) {
    _derived = _Derived(children, computeSize,
        recursively: recursively, filter: filter);
  }
